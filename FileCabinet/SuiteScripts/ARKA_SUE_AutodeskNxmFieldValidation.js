...
[
    ...
    'N/ui/message',
    'N/search'
], function (query, record, bbeLib, message, search) {
    ...
    function beforeSubmit(context) 
    {
        ...
        if (itemLineDetails.itemsArr.length)
        {
            ...
            {
                let itemLineAutodesk = itemLineDetails.itemDetailsArr
                                        .filter(lineObj => Object.keys(itemDetailsObj).includes(lineObj.itemId)) //filter only autodesk line from transaction line
    ...
    function isAuthorisationConfigure(subsidiary) 
    {
        ...
        authObj.missingAuth = !searchResult.custrecord_autodesk_csn || !searchResult.custrecord_consumer_key || !searchResult.custrecord_autodesk_consumer_secret || !searchResult.custrecord_autodesk_callback_url; 
    ...
    function validateLineFields(itemLines)
    {
        ...
        itemLines.forEach(lineData => {
            ...
            {
                if (!lineData.startDate && [QUOTE_SCENARIO.NEW, QUOTE_SCENARIO.COTERM_NEW, QUOTE_SCENARIO.PREMIUM_SUBS].includes(lineData.autodeskScenario))
                {
                    missingLineFields.push('Item Start Date');
                }
                if (!lineData.endDate && [QUOTE_SCENARIO.EXTEND, QUOTE_SCENARIO.PREMIUM_SUBS].includes(lineData.autodeskScenario))
                {
                    missingLineFields.push('Item End Date');
                }
                if (!lineData.serialNumber && [QUOTE_SCENARIO.RENEWAL, QUOTE_SCENARIO.SWITCHTERM, QUOTE_SCENARIO.EXTEND].includes(lineData.autodeskScenario))
                {
                    missingLineFields.push('Serial Number');
                }
                if (!lineData.refSerialNumber && [QUOTE_SCENARIO.COTERM_NEW, QUOTE_SCENARIO.SWITCH].includes(lineData.autodeskScenario))
                {
            ...
            if (missingLineFields.length)
                errorMessage.push(`Item Line ${lineData.line}: Missing fields: ${missingLineFields.join(', ')} ${(lineData.autodeskScenario? `for the quote scenario: ${QUOTE_SCENARIO_NAME[lineData.autodeskScenario]}`: '')}`) 
    ...
    function validateItems(itemDetails)
    {
        ...
        itemDetails.forEach(itemObj => {
            ...
            if (missingFields.length) errorMessage.push(`Please enter a value for fields ${missingFields.join(', ')} for the item ${itemObj.itemName}`);
