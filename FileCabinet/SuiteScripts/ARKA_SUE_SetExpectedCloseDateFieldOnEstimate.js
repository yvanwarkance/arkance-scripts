 ...
 */, (log, record, serverWidget, search) => {
    ...
    const fetchEstimateData = async (type, estimateId) => {
        ...
        const estimateSearch = await search.create.promise({
            type: 'transaction',
            filters: [['type', 'anyof', 'Estimate'], 'AND', ['internalid', 'is', estimateId.toString()]],
            columns: [
    ...
    const afterSubmit = async (scriptContext) => {
        ...
        try {
            ...
            if (estimateData.length != 0) {
                ...
                const isFinalizedStage = LIST_OF_FINALIZED_STAGES.some(
                    (availableStage) => availableStage.correspondingID === parseInt(stage)
                )
                if (isFinalizedStage) {
                    // Get the closed date
                    const closedDate = estimateData[0].getValue('closedate')
