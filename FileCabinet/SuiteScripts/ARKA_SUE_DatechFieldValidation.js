...
[
    ...
    function beforeSubmit_validateDatechFields(context) {
        ...
        if (!subsidiary) {
            ...
            var searchType =
                transObj.type === record.Type.SALES_ORDER
                    ? search.Type.SALES_ORDER
                    : transObj.type === record.Type.ESTIMATE
        ...
        if (subsidiaries.indexOf(subsidiary) > -1 && !datechVendorIds) {
            ...
            if (subsidiary == subsidiaries[0]) {
                setErrorMessage(transObj, [
                    "Erreur: Authorisation n'est pas configuré correctement. Veuillez contacter l'administrateur.",
                ])
            } else {
                setErrorMessage(transObj, [
                    'Error: Authorisation record not configure properly. Please contact the administrator.',
                ])
            }
            return
        }
    ...
    function fetchItemsByIds(itemIdArr, fetchVendor) {
        ...
        var searchObj = search.create({
            ...
            filters: [
                ...
                'AND',
                ['custitem_product_line', 'anyof', AUTODESK_PRODUCT_LINE],
                'AND',
                ['custitem_software_product', 'noneof', AUTODESK_FLEX_SOFTPROD, FLEX_SOFTPROD],
                'AND',
                ['custitem_autodesk_offering_id', 'isempty', ''],
                'AND',
                filterItemIds,
            ],
            columns: columnsArr,
    ...
    function checkItemType(transObj, autodeskType, msgArr, additionalFieldsTocheck) {
        ...
        if (subsidiary != subsidiaries[3] || subsidiary != subsidiaries[4]) {
            for (var i = 0; i < numItemLines; i++) {
                var checkContractNumber =
                    additionalFieldsTocheck && additionalFieldsTocheck.indexOf(AUTODESK_CONTRACT_NUMBER) >= 0
                var checkRenewalNumber = additionalFieldsTocheck && additionalFieldsTocheck.indexOf(RENEWAL_NUMBER) >= 0
                var checkSerialNumberToUpdate =
                    additionalFieldsTocheck && additionalFieldsTocheck.indexOf(SERIAL_NUMBER_TO_UPDATE) >= 0
                var checkUpgradeFromSerial =
                    additionalFieldsTocheck && additionalFieldsTocheck.indexOf(UPGRADE_FROM_SERIAL) >= 0
                checkSwitchItem = additionalFieldsTocheck && additionalFieldsTocheck.indexOf(SWITCH_ITEM) >= 0
                var checkAutoDeskTypeByLine =
                    additionalFieldsTocheck && additionalFieldsTocheck.indexOf(DOUBLE_SCENARIO) >= 0
                var checkComments = additionalFieldsTocheck && additionalFieldsTocheck.indexOf(COMMENTS) >= 0
                var checkSwitchCategory =
                    additionalFieldsTocheck && additionalFieldsTocheck.indexOf(SWITCH_COLLECTION_CATEGORY) >= 0
                ...
                if (checkContractNumber) {
                    ...
                    if (!autodeskContractNumber) {
                        if (subsidiary == subsidiaries[0]) {
                            msgArr.push(
                                'Article : ' + itemName + ' doit avoir le champ (Autodesk Contract Number) rempli'
                            )
                        } else {
                            msgArr.push(
                                'Item: ' + itemName + ' must have the (Autodesk Contract Number) field filled in'
                            )
                        }
                    }
                }
                ...
                if (checkSerialNumberToUpdate) {
                    ...
                    if (!serialNumberToUpdate) {
                        if (subsidiary == subsidiaries[0]) {
                            msgArr.push(
                                'Article : ' + itemName + ' doit avoir le champ (Serial Number To Be Updated) rempli'
                            )
                        } else {
                            msgArr.push(
                                'Item: ' + itemName + ' must have the (Serial Number To Be Updated) field filled in'
                            )
                        }
                    }
                }
                ...
                if (checkUpgradeFromSerial) {
                    ...
                    if (!upgradeFromSerial) {
                        if (subsidiary == subsidiaries[0]) {
                            msgArr.push(
                                'Article : ' + itemName + ' doit avoir le champ (Upgrade From Serial Number) rempli'
                            )
                        } else {
                            msgArr.push(
                                'Item: ' + itemName + ' must have the field (Upgrade From Serial Number) filled in'
                            )
                        }
                    }
                }
                ...
                if (checkSwitchCategory) {
                    ...
                    if (
                        ...
                        if (subsidiary == subsidiaries[0]) {
                            msgArr.push(
                                'Article : ' + itemName + ' doit avoir un license du type switch to collection.'
                            )
                        } else {
                            msgArr.push(
                                'Item: ' + itemName + ' must have a license type for type switch to collection.'
                            )
                        }
                    }
                }
    ...
    function validateDatechFieldsPerLine(transObj) {
        ...
        for (var i = 0; i < numItemLines; i++) {
            ...
            if (!lineOrder) {
                if (subsidiary == subsidiaries[0]) {
                    msgArr.push(
                        'Article : ' + itemName + ' doit être associé à un scénario de commande Datech sur la ligne'
                    )
                } else {
    ...
    function validateDatechFieldsPerLineScenario(transObj, lineId, orderScenario, searchResultByItem, msgArr) {
        ...
        var datechError = {
            body: {
                shipaddress: [
                    "L'utilisateur final (End User) doit avoir un adresse d'expédition",
                    'End User must have a shipping address',
                ],
                shipzip: [
                    "L'utilisateur final (End User) doit avoir un postal code associé à l'adresse",
                    'The End User must have a postal code associated with the address',
                ],
                shipcountry: [
                    "L'utilisateur final (End User) doit avoir un pays associé à l'adresse",
                    'The End User must have a country associated with the address',
                ],
            },
            customer: {
                custentity_autodesk_csn_false: [
                    "L'utilisateur final (End User) ne doit pas disposer d'Autodesk CSN",
                    'End User must not have Autodesk CSN',
                ],
                custentity_autodesk_csn_true: [
                    "L'utilisateur final (End User) doit disposer d'Autodesk CSN",
                    'End User must have an Autodesk CSN',
                ],
                companyname: [
                    "L'utilisateur final (End User) doit avoir un dénomination sociale",
                    'The End User must have a company name',
                ],
                shipcity: [
                    "L'utilisateur final (End User) doit avoir une ville associé à l'adresse",
                    'The End User must have a city associated with the address',
                ],
            },
            item: {
                custitem_autodesk_order_type_1: [
                    'Article : itemName? doit être de type Autodesk lineAutodeskType?',
                    'Item : itemName? should be of type lineAutodeskType?',
                ],
                custitem_autodesk_order_type_5: [
                    'Article : itemName? doit être de type Autodesk lineAutodeskType?',
                    'Item : itemName? should be of type lineAutodeskType?',
                ],
                custitem_license_type: [
                    'Article : itemName? doit avoir un license du type switch to collection.',
                    'Item: itemName? must have a license type for type switch to collection.',
                ],
                custitem_switch_true: [
                    'Au moins un article doit être de type (Switch <M2S>)',
                    'At least one article must be of type (Switch <M2S>)',
                ],
            },
            line: {
                custcol_autodesk_contract_number: [
                    'Article : itemName? doit avoir le champ (Autodesk Contract Number) rempli',
                    'Item: itemName? must have the (Autodesk Contract Number) field filled in',
                ],
                custcol_serial_number: [
                    'Article : itemName? doit avoir le champ (Serial Number) rempli',
                    'Item: itemName? must have the (Serial Number) field filled in',
                ],
                custcol_renewal_number: [
                    'Article : itemName? doit avoir le champ (Renewal Number) rempli',
                    'Item: itemName? must have the (Renewal Number) field filled in',
                ],
                custcol_autodesk_comments: [
                    'Article : itemName? doit avoir le champ (Commentaires) rempli',
                    'Item : itemName? must have the field (Comments) filled in.',
                ],
            },
        }
        ...
        var datechFieldValidation = {
            ...
            6: {
                ...
                item: [{ custitem_autodesk_order_type: ADSR }],
            }, // Renewal
            8: {
                body: [],
            ...
            9: {
                ...
                customer: [{ custentity_autodesk_csn: true }],
                item: [
                    { custitem_autodesk_order_type: ADSA },
                    {
                        custitem_license_type: [
                            SUBSCRIPTION_SWITCH_FROM_CATEGORY_1,
                            SUBSCRIPTION_SWITCH_FROM_CATEGORY_2,
                        ],
                    },
                ],
            },
