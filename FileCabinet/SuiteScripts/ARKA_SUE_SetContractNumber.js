 ...
 */
define(['N/log', 'N/record', './BBE_LIB_GenericFunctionsSS2', 'N/search'], function (log, record, bbeLib, search) {
    ...
    function setAutodeskContratNumber(scriptContext) {
        ...
        if (
            ...
            if (newRecord.type == record.Type.SALES_ORDER) {
                ...
                if (orderNumber) {
                    if (scriptContext.type == scriptContext.UserEventType.CREATE && status == PENDING_BILLING) {
                        newRecord.setValue({
        ...
        if (scriptContext.type == scriptContext.UserEventType.CREATE && newRecord.type == record.Type.INVOICE) {
