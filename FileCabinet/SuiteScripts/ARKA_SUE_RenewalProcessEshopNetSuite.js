...
define([
  ...
  "N/url",
  "N/runtime",
], function (log, record, bbeLib, search, url, runtime) {
  ...
  function afterSubmit(scriptContext) {
    ...
    try {
      if (newRecord.type == record.Type.ESTIMATE) {
        ...
        if (
          estimateRecord.getValue("custbody_order_type") ==
            CONTRACT_RENEWAL_ID &&
          estimateRecord.getValue("custbody_eshopmake_copy") == true
        ) {
          makeCopyEstimate(
            scriptContext,
            estimateRecord.getValue("custbody_end_user")
          ); // 0 units
        }
      }
      ...
      bbeLib.logError({
        err: e,
        title:
          "Could Not Make a copy of Estimate or set it to void or update customer record",
