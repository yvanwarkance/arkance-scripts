...
define(['N/currentRecord', 'N/ui/message', 'N/search', 'N/record', 'N/ui/dialog', 'N/runtime'], function (
    ...
    function pageInit(scriptContext) {
        ...
        // ARK-323
        ;(currentType == 'opportunity' || currentType == 'estimate') && checkboxTender == true
            ? setFieldDisabled('probability', false)
            : setFieldDisabled('probability', true)
    ...
    function fieldChanged(context) {
        try {
            ...
            if (
                ...
                (fieldName == 'custbody_startdate' || fieldName == 'custbody_tran_term_in_months')
            ) {
                let terms = currentRec.getValue({ fieldId: 'custbody_tran_term_in_months' })
    ...
    function validateLine(context) {
        ...
        if (currentType == 'opportunity' && sublistId == 'item' && itemType == 'NonInvtPart') {
            ...
            let lineStartDate = currentRec.getCurrentSublistValue({
                sublistId: 'item',
                fieldId: 'custcol_swe_contract_start_date',
            })
            let newlineStartDate = formatDate(lineStartDate)
            let lineEndDate = currentRec.getCurrentSublistValue({
                sublistId: 'item',
                ...
                switch (true) {
                    case newStartDateHeader > newlineStartDate:
                        dialog.alert({
                            ...
                            message:
                                'The Contract Item Start Date is earlier than the transaction Start Date. \n Please enter a valid Contract Item Start Date.',
                        })
                        return false
                    ...
                    case newEndDateHeader < newlineEndDate:
                        dialog.alert({
                            title: '5565205-sb1.app.netsuite.com says',
                            message:
                                'The Contract Item End Date is later than the transaction End Date. \n Please enter a valid Contract Item End Date.',
                        })
                        return false
                }
            }
