 ...
 * GLOBAL VARIABLES
 */
if(!SWE.R01B.CS.TranLines.Variables) SWE.R01B.CS.TranLines.Variables =
	{
		...
		arrMaintenanceBasisCat : SWE.Renewals.Library.splitList(SWE.Renewals.Library.Constants.ITEMCATS_FOR_MAINTENANCE_BASIS),
...
SWE.R01B.CS.TranLines.Variables.DECIMAL_PLACES_LISTRATE = SWE.Library.misc.isUndefinedNullOrEmpty(SWE.R01B.CS.TranLines.Variables.DECIMAL_PLACES_LISTRATE) || parseInt(SWE.R01B.CS.TranLines.Variables.DECIMAL_PLACES_LISTRATE) < 0 ?
		SWE.R01B.CS.TranLines.Variables.NUMBER_OF_PREFERRED_DECIMAL : parseInt(SWE.R01B.CS.TranLines.Variables.DECIMAL_PLACES_LISTRATE);
SWE.R01B.CS.TranLines.Variables.DECIMAL_PLACES_RATE = SWE.Library.misc.isUndefinedNullOrEmpty(SWE.R01B.CS.TranLines.Variables.DECIMAL_PLACES_RATE) || parseInt(SWE.R01B.CS.TranLines.Variables.DECIMAL_PLACES_RATE) < 0 ?
		SWE.R01B.CS.TranLines.Variables.NUMBER_OF_PREFERRED_DECIMAL : parseInt(SWE.R01B.CS.TranLines.Variables.DECIMAL_PLACES_RATE);
        ...
        for (var idx = 1; idx <= nlapiGetLineItemCount('item'); idx++) {
            ...
            if (bIsItemGroup || bIsProjItem) {
                ...
                if (SWE.Library.misc.isUndefinedNullOrEmpty(flCurListRate)) {
            		...
            		// Issue 238123
            		if(!SWE.Library.misc.isUndefinedNullOrEmpty(nlapiGetLineItemValue('item','custcol_item_category', idx)) &&
                ...
                if(SWE.R01B.CS.TranLines.validateListRate(tranLine, idx) == false) { return false; }
                if(SWE.R01B.CS.TranLines.validateItemDatesAgainstContractDates(tranLine, stTranStartDt, stTranEndDt, idx) == false) { return false; }
                if(SWE.R01B.CS.TranLines.validateItemTermsAndDates(tranLine, idx) == false) { return false; }
            }
            ...
            if (MSPricingOption == constants.MS_MODEL_PERCENTAGE || MSPricingOption == constants.MS_MODEL_PERCENTAGE_NET) {
                ...
                if (tranLine.isMaintenanceCategoryType()) {
                    ...
                    if (SWE.R01B.CS.TranLines.processSupportOrMaintenance(globalVar.arrMaintenanceBasisCat, true) != true) {
                        return false;
                    }
                    ...
                    nlapiSelectLineItem('item', idx);
                    if (SWE.R01B.CS.TranLines.processSupportOrMaintenance(globalVar.arrSupportBasisCat, false) != true) {
                        return false;
                    }
                    if(tranLine.getPrice() == -1){
                    	SWE.R01B.CS.TranLines.updateExtendedRate(tranLine);
...
SWE.R01B.CS.TranLines.validateLine_TranLines = function(stListName) {
    ...
    if(SWE.Renewals.Library.isRecordTypeRMA() && SWE.Renewals.Library.itemCategoryRequiresContract(tranLine.getCategory())){
        ...
        if (stListName == "item") {
            ...
            if (!SWE.Renewals.Library.isRecordTypeOpportunity()) {
                if (tranLine.isProcessableCategoryType()) {
                    if (!SWE.Renewals.Library.isRecordTypeCreditMemo()) { // credit memo has no header start and end dates
                        if(!SWE.R01B.CS.TranLines.isValidTransactionDates(stStartDate, stEndDate)) { return false; }
                    }
                    ...
                    if( !SWE.R01B.CS.TranLines.isValidContractItemDates(tranLine, stStartDate, stEndDate) ) { return false; }

                } else if ( tranLine.isPerpetualCategoryType() || tranLine.isHardwareCategoryType() || // for Perpetual or Hardware item category
                            (tranLine.getCategory() == constants.ITEMCAT_TYPE_OTHER) ) { // Issue 204869 : SWV-CR | Compatibility with VSOE Feature >
            	...
            	if (SWE.Library.misc.isUndefinedNullOrEmpty(tranTerm)){ tranTerm = 12; }
    ...
    if (globalVar.isR01CCSEnabled) {
    	...
    	globalVar.flOrigListRate = SWE.Renewals.Library.parseFloatOrZero(nlapiGetCurrentLineItemValue('item', 'custcol_list_rate'));
        var stItemType = tranLine.getItemType();
...
//                    }
                    ...
                    if (tranLine.isSupportCategoryType()
                        ...
                        if (SWE.Library.misc.isUndefinedNullOrEmpty(flTermInMonths)) {
                    		...
                    		if(SWE.Renewals.Library.isRecordTypeInvoice()){
                    			...
                    			if(SWE.Library.misc.isUndefinedNullOrEmpty(tranLine.getListRate())){
                    				...
                    				tranLine.setListRate(SWE.Library.misc.getRoundedDec(tranLine.getExtendedRate(), 
                    				    globalVar.DECIMAL_PLACES_LISTRATE), true, true);
                    				...
                    				if(globalVar.createdFromRecType == 'salesorder'){
                    					...
                    					tranLine.setExtendedRate(SWE.Library.misc.getRoundedDec(tranLine.getListRate(), globalVar.DECIMAL_PLACES_LISTRATE), true, true);
                    				} else {
                                        ...
                                        tranLine.setListRate(SWE.Library.misc.getRoundedDec(tranLine.getExtendedRate(), globalVar.DECIMAL_PLACES_LISTRATE), true, true);
                    				}
                    			}
                    		} else {
                                tranLine.setPrice('-1', false, true);
                            ...
                            tranLine.setListRate(SWE.Library.misc.getRoundedDec(flListRate, globalVar.DECIMAL_PLACES_LISTRATE), true,true);
                        }
                    } else {
...
//                              flRate = (stItemPricingType == constants.ITEM_PRICING_ANNUAL && (!tranLine.isPerpetualCategoryType())) ? (Math.round((stOrigListRate / 12) * 10000) / 10000) : stOrigListRate;
                                flRate = (stItemPricingType == constants.ITEM_PRICING_ANNUAL && (!tranLine.isPerpetualCategoryType())) ?
                                          SWE.Library.misc.getRoundedDec(stOrigListRate / 12, globalVar.DECIMAL_PLACES_LISTRATE) : stOrigListRate;
...
SWE.R01B.CS.TranLines.processTermLicense = function(stListName, iLineIndex) {
    ...
    if (stPriceLevel == null){    	
        ...
        if (stPriceLevel != -1) {
            ...
            var origListRate = nlapiGetCurrentLineItemValue('item','custcol_swe_orig_list_rate');
                	...
                	nlapiLogExecution('DEBUG', 'processTermLicense', 'globalVar.flOrigListRate1:' + globalVar.flOrigListRate);
            ...
            if (!SWE.Library.misc.isUndefinedNullOrEmpty(globalVar.flOrigListRate)) {
            	...
            	if (stItemPricingType == constants.ITEM_PRICING_ANNUAL)
    ...
    if (SWE.Library.misc.isUndefinedNullOrEmpty(flTermInMonths)) {
        ...
        if (SWE.Library.misc.isUndefinedNullOrEmpty(dtStartDate)
            ...
            if (SWE.Renewals.Library.isRecordTypeOpportunity()) {
                ...
                if (SWE.Renewals.Library.isRecordTypeCreditMemo() && SWE.Library.misc.isUndefinedNullOrEmpty(stContractID)) {
                    return true;
                } else {
    ...
    if (stPriceLevel == -1) {
		...
		flFinalCustomRate = SWE.Library.misc.getRoundedDec(flFinalCustomRate, SWE.R01B.CS.TranLines.Variables.DECIMAL_PLACES_RATE);
    ...
    if (stItemPricingType == constants.ITEM_PRICING_ACTUAL_PRICE) {
        ...
        nlapiSetCurrentLineItemValue(stListName, 'custcol_list_rate', 
            SWE.Library.misc.getRoundedDec(flNewRate, globalVar.DECIMAL_PLACES_LISTRATE), false, true);
    } else {
        ...
        if (flTermInMonths == 0) {
          ...
          //nlapiSetCurrentLineItemValue(stListName, "custcol_list_rate", Math.round(flNewRate / flTermInMonths * 10000)/10000, false, true);
            nlapiSetCurrentLineItemValue(stListName, 'custcol_list_rate', SWE.Library.misc.getRoundedDec(flNewRate / flTermInMonths, globalVar.DECIMAL_PLACES_LISTRATE), true,true);       
        }
    }
  ...
  //nlapiSetCurrentLineItemValue(stListName, "rate", Math.round(flNewRate*10000)/10000, false, true);
    ...
    if ((SWE.Renewals.Library.parseFloatOrZero(globalVar.flOrigRate) != SWE.Renewals.Library.parseFloatOrZero(flNewRate) && SWE.Renewals.Library.isRecordTypeInvoice()) || !SWE.Renewals.Library.isRecordTypeInvoice()) {
		...
		nlapiSetCurrentLineItemValue(stListName, 'rate', SWE.Library.misc.getRoundedDec(flNewRate, SWE.R01B.CS.TranLines.Variables.DECIMAL_PLACES_RATE), true,true); //round off in updateExtendedRate()
    }
    return true;
};
            ...
            case constants.MS_MODEL_PERCENTAGE_NET:
                ...
                if (!SWE.Library.misc.isUndefinedNullOrEmpty(flPercentage)) {
                    ...
                    if (!SWE.Library.misc.isUndefinedNullOrEmpty(stMSType)) {
                        ...
                        if (!SWE.Library.misc.isUndefinedNullOrEmpty(stMSPercent)) {
                            nlapiSetCurrentLineItemValue('item', 'custcol_mtce_support_percent', stMSPercent, true, true);
                ...
                switch (MSPricingOption) {
                    case constants.MS_MODEL_PERCENTAGE:
                        ...
                        break;
                    case constants.MS_MODEL_PERCENTAGE_NET:
                        flTotalAmt = SWE.Renewals.Library.computeTotalNetAmount(arrBasisCat, stProdLine, stItemPricingType);
                        nlapiSetCurrentLineItemValue('item', 'custcol_swe_ms_basis_amount', flTotalAmt);
                        break;
                }
    ...
    if (globalVar.isR01CCSEnabled) {
        ...
        if (stListName == 'item' && stFieldName == 'item') {
            ...
            if (stItemType != 'Discount' &&
                ...
                if ((SWE.Renewals.Library.searchInList(globalVar.arrMaintCat, stItemCat) || SWE.Renewals.Library.searchInList(globalVar.arrSupportCat, stItemCat)) && globalVar.MS_CUST_INLINE_DISC) {
                	SWE.R01B.CS.TranLines.applyDefaultCustomerDiscount();
                }
                else {
                    if (!(SWE.Renewals.Library.searchInList(globalVar.arrMaintCat, stItemCat) || SWE.Renewals.Library.searchInList(globalVar.arrSupportCat, stItemCat)) && globalVar.DFLT_CUST_DISC_ON_TRAN_LINE) {
                    	SWE.R01B.CS.TranLines.applyDefaultCustomerDiscount();
                    }
                }
                ...
                if (SWE.Renewals.Library.isRecordTypeRMA()) {
                    ...
                    if (!SWE.Library.misc.isUndefinedNullOrEmpty(stContractId)) {
                        var tranLine = new TranLine(null);
                        var flDiscountRate = SWE.R01B.CS.TranLines.getContractItemDiscount(stContractId, tranLine, 'custrecord_ci_original_discount', 'min');                        
            ...
            if (nlapiGetCurrentLineItemValue('item', 'item') == '') {
                if (!SWE.Renewals.Library.isRecordTypeOpportunity()) { //Quick fix for Issue 215719
...
SWE.R01B.CS.TranLines.updateContractItemDateFields = function(tranLine, start, end, term){
 ...
 // Issue 358706 | mbautista - default dates to dates in contract header
    if(SWE.Library.misc.isUndefinedNullOrEmpty(iTermContractItem) && SWE.Library.misc.isUndefinedNullOrEmpty(stContractStartDate)
        ...
        if (SWE.Library.misc.isUndefinedNullOrEmpty(iTermContractItem)) {
            ...
            iTermContractItem = SWE.Library.misc.getRoundedDec(iTermContractItem, globalVar.NUMBER_OF_CONTRACT_TERM_DECIMAL);
...
SWE.R01B.CS.TranLines.isValidContractItemDates = function(tranLine, startDate, endDate){
    ...
    if ( (!SWE.Library.misc.isUndefinedNullOrEmpty(stContractStartDate))
    	...
    	var isForRenewalContract = (orderType != constants.ORDER_TYPE_RENEWAL && 
    	    !SWE.Library.misc.isUndefinedNullOrEmpty(stContractID)) ? SWE.R01B.CS.TranLines.hasOriginalContract(orderType) : false;
    	var applyFlexibleStart = globalVar.ENABLE_FLEXIBLE_START == 'T' && (orderType == constants.ORDER_TYPE_RENEWAL || (orderType == constants.ORDER_TYPE_UPSELL && isForRenewalContract));
    	var minStart = applyFlexibleStart && SWE.Library.dates.compare(startDate, globalVar.currentDate) > -1 ? globalVar.currentDate : startDate;
        ...
        if (SWE.Library.dates.inRange(stContractStartDate, minStart, endDate) != true) {
            if (SWE.Library.dates.compare(stContractStartDate, minStart) == -1) {
                alert( SWE.Renewals.Library.isRecordTypeCreditMemo() ? SWE.Library.constants.MSGS_CONSTANTS[47] : 
                	(applyFlexibleStart ? SWE.Library.constants.MSGS_CONSTANTS[68] : SWE.Library.constants.MSGS_CONSTANTS[2]));
            } else {
                alert( SWE.Renewals.Library.isRecordTypeCreditMemo() ? SWE.Library.constants.MSGS_CONSTANTS[48] : SWE.Library.constants.MSGS_CONSTANTS[3]);
            }
            return false;
        }
    }
    ...
    if (!SWE.Library.misc.isUndefinedNullOrEmpty(stContractEndDate)
        ...
        if (SWE.Library.dates.inRange(stContractEndDate, startDate, endDate) != true) {
            if (SWE.Library.dates.compare(stContractEndDate, startDate) == -1) {
                alert( SWE.Renewals.Library.isRecordTypeCreditMemo() ? SWE.Library.constants.MSGS_CONSTANTS[49] : SWE.Library.constants.MSGS_CONSTANTS[5]);
            } else {
                alert( SWE.Renewals.Library.isRecordTypeCreditMemo() ? SWE.Library.constants.MSGS_CONSTANTS[50] : SWE.Library.constants.MSGS_CONSTANTS[6]);
            }
            return false;
        }
    }
...
SWE.R01B.CS.TranLines.updateExtendedRate = function(tranLine) {
    ...
    if (!SWE.Library.misc.isUndefinedNullOrEmpty(flExtendedRate) && flExtendedRate != 0) {
        ...
        if (globalVar.isR01DCSEnabled && globalVar.ALLOW_DISCOUNT) { //Apply discount
        ...
        if (tranLine.isItemTypeProcessable()) {
            if (tranLine.isTermCategoryType() || tranLine.isMaintenanceCategoryType() || tranLine.isSupportCategoryType()) {
                ...
                if (!SWE.Library.misc.isUndefinedNullOrEmpty(flCurListRate) && flCurListRate != 0) {
                    ...
                    tranLine.setListRate(SWE.Library.misc.getRoundedDec(flCurListRate/12, globalVar.DECIMAL_PLACES_LISTRATE), false, true);
                }
            }
        }
    }
...
SWE.R01B.CS.TranLines.contractStartDateFieldChangeHandler = function(stListName, iLineIndex, tranLine)
{
    ...
    if (tranLine.getCategory() != constants.ITEMCAT_TYPE_LICENSE_PERP && tranLine.getCategory() != constants.ITEMCAT_TYPE_HARDWARE 
    	...
    	if(!SWE.Library.misc.isUndefinedNullOrEmpty(endDate)){
    		term = SWE.Library.misc.getRoundedDec(SWE.Library.dates.dateDiff(stContractStartDate, endDate), globalVar.NUMBER_OF_CONTRACT_TERM_DECIMAL);
        	tranLine.setItemTermInMonths(term, false);
    	}else{
    		var tranEnd = SWE.Renewals.Library.isRecordTypeRMA() ? nlapiGetFieldValue('custbody_swe_rma_header_end_date') : nlapiGetFieldValue('enddate');
    		if(!SWE.Library.misc.isUndefinedNullOrEmpty(term)){
	        	...
	        	tranLine.setItemTermInMonths(SWE.Library.misc.getRoundedDec(term, globalVar.NUMBER_OF_CONTRACT_TERM_DECIMAL), false);
    		}
...
SWE.R01B.CS.TranLines.contractEndDateFieldChangeHandler = function(stListName, iLineIndex, tranLine)
{
    ...
    if (tranLine.getCategory() != constants.ITEMCAT_TYPE_LICENSE_PERP && tranLine.getCategory() != constants.ITEMCAT_TYPE_SERVICES_PERP) {
        ...
        if (isCustomBlocked) {
            if (!isCustomMSAllowed) {
                ...
                if (!tranLine.isMaintenanceCategoryType() && !tranLine.isSupportCategoryType()) {
                    if (tranLine.getPrice() == "-1"){//(!globalVar.CUSTOM_PRICE_CHECK_DONE) {
                    	globalVar.CUSTOM_PRICE_CHECK_DONE = true;
...
{   
    ...
    if ((SWE.Renewals.Library.isRecordTypeInvoice() || ((SWE.Renewals.Library.isRecordTypeSalesOrder())&&(stOrderStatus == constants.SO_STATUS_PENDING_FULFILLMENT || stOrderStatus == constants.SO_STATUS_BILLED))) && dtContractStartDate != '')  {
        nlapiSetLineItemDisabled('item', 'custcol_swe_contract_item_term_months', true);
...
SWE.R01B.CS.TranLines.getContractItemDiscount = function( stContractId, tranLine, stFieldName, stGroup ){
    ...
    try {
        ...
        var var_url_servlet = nlapiResolveURL('SUITELET', 'customscript_swe_optimize_suitelet2', 'customdeploy_swe_optimize_suitelet2', true);
        nlapiLogExecution('DEBUG', 'var_url_servlet', 'Value: ' + var_url_servlet + ' ---- stParameters: ' + stParameters);
...
SWE.R01B.CS.TranLines.isValidRMAItem = function(tranLine, lineNo){
    ...
    try {
        ...
        var CHECK_RATE = ( ((tranLine.isSupportCategoryType() || tranLine.isMaintenanceCategoryType())
                && (MSPricingOption == constants.MS_MODEL_PERCENTAGE || MSPricingOption == constants.MS_MODEL_PERCENTAGE_NET)) ? 'F' : 'T' );
        ...
        if (lineNo == null){
            ...
            iOtherQuantity = SWE.Renewals.Library.computeItemQuantity( stItemId, nlapiGetCurrentLineItemIndex('item'), flListRate, bRenewalsExclusion); // Get Quantity of item in other line items
            iOtherQuantityTotal = SWE.Renewals.Library.computeItemQuantity( stItemId, nlapiGetCurrentLineItemIndex('item'), flListRate, true); // disregard list rate
        } else {
        ...
        var stOthParameter = '&RenewalsExclusion=' + bRenewalsExclusion
                             + '&OtherQuantity=' +  parseFloat(iOtherQuantity)
                             + '&OtherQuantityTotal=' +  parseFloat(iOtherQuantityTotal);
...
SWE.R01B.CS.TranLines.isValidRMAItemError = function (stResult, lineNo, bRenewalsExclusion)
{
    ...
    if (stResult == 'F')
    {  //item is not present
        alert(lineNo != null ? SWE.Library.messages.displayMessage(SWE.Library.constants.MSGS_CONSTANTS[35], lineNo) : SWE.Library.constants.MSGS_CONSTANTS[31]);
        return false;
    }

    if ((stResult == 'R') && (bRenewalsExclusion == false))
    {  //Do not validate rate of rma item if not included for renewal
        alert(lineNo != null ? SWE.Library.messages.displayMessage(SWE.Library.constants.MSGS_CONSTANTS[36], lineNo) : SWE.Library.constants.MSGS_CONSTANTS[32]);
        return false;
    }

    if (stResult == 'Q')
    {  //quantity error
        alert(lineNo != null ? SWE.Library.messages.displayMessage(SWE.Library.constants.MSGS_CONSTANTS[37], lineNo) : SWE.Library.constants.MSGS_CONSTANTS[33]);
        return false;
    }

    if (stResult == 'M')
    { //terms error
        alert(lineNo != null ? SWE.Library.messages.displayMessage(SWE.Library.constants.MSGS_CONSTANTS[45], lineNo) : SWE.Library.constants.MSGS_CONSTANTS[46]);
        return false;
    }
...
SWE.R01B.CS.TranLines.validateItemDatesAgainstContractDates = function(tranLine, stTranStartDt, stTranEndDt, idx){
	...
	if (!SWE.Renewals.Library.isRecordTypeOpportunity()) {
    	...
    	var isForRenewalContract = 
    	    !SWE.Library.misc.isUndefinedNullOrEmpty(stContractID) ? SWE.R01B.CS.TranLines.hasOriginalContract(orderType) : false;
    	var applyFlexibleStart = globalVar.ENABLE_FLEXIBLE_START == 'T' && (orderType == constants.ORDER_TYPE_RENEWAL || (orderType == constants.ORDER_TYPE_UPSELL && isForRenewalContract));
    	var minStart = applyFlexibleStart && SWE.Library.dates.compare(stTranStartDt, globalVar.currentDate) > -1 ? globalVar.currentDate : stTranStartDt;

        var iResult = SWE.Library.dates.compare(minStart,stContractStartDt) == 1;
        if (iResult) {
        	applyFlexibleStart ? alert(SWE.Library.messages.displayMessage(SWE.Library.constants.MSGS_CONSTANTS[69], idx))
...
SWE.R01B.CS.TranLines.validateDelete_TranLines = function(type){
	...
	if( type=='item' ) {
		...
		var isGeneratedByCrEnhancement = SWE.Library.misc.getHiddenFieldValue(SWE.Parameters.IS_GENERATED_BY_CR_ENHANCEMENT)=='T';
	    ...
	    if( isApplyUpsellDownsellEnabled && isGeneratedByCrEnhancement ) {
	    	...
	    	if( !SWE.Library.misc.isUndefinedNullOrEmpty(stContractItemId) ) {
	    		...
	    		if(confirm(SWE.Library.constants.MSGS_CONSTANTS[63])){
	    			...
	    			if (SWE.Renewals.Library.searchInList(globalVar.arrMaintCat, itemCategory) || SWE.Renewals.Library.searchInList(globalVar.arrSupportCat, itemCategory)) {
	    				
		    			var itemsForDownsell = SWE.Library.misc.getHiddenFieldValue( 'itemsForDownsell' );
		    			
	    				itemsForDownsell += SWE.Library.misc.isUndefinedNullOrEmpty(itemsForDownsell) ? stContractItemId : "," + stContractItemId;
