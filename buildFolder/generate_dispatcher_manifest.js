...
try {
    ...
    for (const manifestFile of manifestFiles) {
        ...
        manifest.forEach(script => {
            const scriptName = path.join(manifestDir, script.name);

            // Make sure to remove FileCabinet/SuiteScripts/ from the script name
            const scriptNameWithoutBase = scriptName.replace("FileCabinet\\SuiteScripts\\", "");
            
            manifestContents.push({
                ...script,
                name: scriptNameWithoutBase
            });
            return true;
        });
    }
    ...
    const groupedByRecordTypeAndScriptType = Object.entries(groupedByRecordType).reduce((acc, [recordType, scripts]) => {
    ...
    const groupedWithDispatchers = Object.entries(groupedByRecordTypeAndScriptType).reduce((acc, [recordType, scriptTypes]) => {
        acc[recordType] = Object.entries(scriptTypes).reduce((typeAcc, [scriptType, scripts]) => {
            const governanceLimit = governanceLimitFiles[scriptType] - 100; // Subtract 100 from limit
