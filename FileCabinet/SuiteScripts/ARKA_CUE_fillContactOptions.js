...
define(['N/query'], (query) => {
	...
	function pageInit(scriptContext) {
		...
		if (currRec.type == 'opportunity' || currRec.type == 'estimate' || currRec.type == 'salesorder') {
			dropDownList.push(
				{
					nsfield: 'custbody_contract_manager_contact', nsnextfield: 'custbody_contract_manager_contact', sourcedfield: 'custbody_end_user',
					genid: 'custpage_contactmanager_selectfield', genName: 'Contract Manager Contact', isMandatory: false, isAddr: false, type: DATATYPE.SELECT
				}
			);

			dropDownList.push(
				{
					nsfield: 'custbody_custom_contactperson', nsnextfield: 'custbody_custom_contactperson', sourcedfield: 'custbody_end_user',
					genid: 'custpage_contactperson_selectfield', genName: 'Contact Person', isMandatory: false, isAddr: false, type: DATATYPE.SELECT
				}
			);
		}
	...
	function fieldChanged(scriptContext) {
		...
		let isFieldFoundArray = dropDownList.filter((dropDownOptions) => dropDownOptions.sourcedfield == currField || dropDownOptions.genid == currField);
			...
			dropDownList.map((dropDownOptions) => {
				...
				if (currField == dropDownOptions.sourcedfield) {
					...
					if (tmpValue) {
						...
						if (selectField) {
							selectField.removeSelectOption({//Empty dropdown
								value: null,
							});
				...
				if (!dropDownOptions.isAddr && currField == dropDownOptions.genid) {//DO NOT replicate on ship address as it overrides old change to set address
	...
	function saveRecord(context) {
		...
		dropDownList.map((dropDownOptions) => {
			if (dropDownOptions.isMandatory)// Validate Mandatory fields
			{
		...
		if (currentRecord.getValue({ fieldId: 'subsidiary' }) == ptSubsidiaryID && currentRecord.getValue({ fieldId: 'type' }) == 'salesord') {
		...
		if ((currentRecord.getValue({ fieldId: 'subsidiary' }) == ptSubsidiaryID) && (currentRecord.getValue({ fieldId: 'type' }) == 'custinvc')) {
		...
		if ((currentRecord.getValue({ fieldId: 'subsidiary' }) == ptSubsidiaryID) && (currentRecord.getValue({ fieldId: 'type' }) == 'custcred')) {
		...
		if ((currentRecord.getValue({ fieldId: 'subsidiary' }) == ptSubsidiaryID) && (currentRecord.getValue({ fieldId: 'type' }) == 'itemship')) {
		...
		if ((currentRecord.getValue({ fieldId: 'subsidiary' }) == ptSubsidiaryID) && (currentRecord.getValue({ fieldId: 'type' }) == 'custpymt')) {
