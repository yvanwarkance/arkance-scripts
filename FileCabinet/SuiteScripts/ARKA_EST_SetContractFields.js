 ...
 *
 */
define(['N/query', 'SuiteScripts/BBE_LIB_GenericFunctionsSS2.js', 'N/record', 'N/ui/message'],// jshint ignore:line

	//function (query, record, BBE_LIB_GenericFunctionsSS2, search) {
	function (query, bbeLib, record, msg) {
		...
		function fieldChangedSetOptions(context) {
			...
			if (fieldName == 'custbody_contract_name') {
				...
				try {
					...
					var contractsSearchQuery = "SELECT custrecordcontrct_mngr_cont_dft FROM customrecord_contracts WHERE id = " + contractId;
					...
					if (resultSet.length == 1) {
						...
						if ((!bbeLib.isNullOrEmpty(contractManagerContactId)) && (contractManagerContactId != 0)) {
							...
							if (!isActiveContact(context, companyId, contractManagerContactId)) {
								showMsg(msg, msg.Type.WARNING, "The contractmanager of the selected contract is inactive. Please select an active contact and also update the contract.")
							...
							showMsg(msg, msg.Type.ERROR, "The contractmanager field of the selected contract is empty. Please select an active contact and also update the contract.")
		...
		function isActiveContact(context, customerId, contactId) {
			...
			var activeContactQuery = "WITH rws AS(" +
				"   SELECT customer.contactlist AS str FROM customer WHERE customer.id=" + customerId +
				")" +
			...
			if (resultSet.length > 1) {
				for (var i = 0; i < resultSet.length; i++) {
					if (resultSet[i].getValue(0) == contactId)
						return true;
				}
			}
			return false;
