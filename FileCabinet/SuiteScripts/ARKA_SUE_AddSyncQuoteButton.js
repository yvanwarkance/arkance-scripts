 ...
 */
define(['./BBE_LIB_GenericFunctionsSS2.js', 'N/query', 'N/url', 'N/ui/message', 'N/record', 'N/ui/serverWidget', 'N/search', 'N/runtime'], function (bbeLib, query, url, message, record, serverWidget, search, runtime) {
    ...
    const CANCELLEDBUTTON_STATUS = [QUOTE_DETAIL_STATUS['DRAFT-CREATED'], QUOTE_DETAIL_STATUS['QUOTED'], QUOTE_DETAIL_STATUS['FINALIZING'], QUOTE_DETAIL_STATUS['UNDER-REVIEW']]
    ...
    function beforeLoad(scriptContext) {
        ...
        {
            var itemTypeSql = " select i.id, itemid,custitem_software_product,custitem_product_line,pl.name,sp.name  " +
            ...
            " where i.id in (" + listItemsArray.join(',') + ") and  custitem_product_line = '" + PROD_AUTODESK + "' and custitem_autodesk_offering_id IS NOT NULL";
    
            //log.debug('itemTypeSql',itemTypeSql)
    
            var resultSet = query.runSuiteQL({  //10 units
                query: itemTypeSql
            });
            ...
            let queryStringCheckConvAuto = " select itemid,custitem_software_product,custitem_product_line,pl.name,sp.name  " +
            " from item i  inner join CUSTOMRECORD_PRODUCT_LINE pl on i.custitem_product_line = pl.id " +
            " inner join CUSTOMRECORD_SOFTWARE_PRODUCTS sp on i.custitem_software_product = sp.id  " +
            " where i.id in (" + listItemsArray.join(',') + ") and  custitem_product_line = '" + PROD_AUTODESK + "' and custitem_autodesk_offering_id IS NULL AND (i.custitem_autodesk_noneligiblenbe = 'F' OR i.custitem_autodesk_noneligiblenbe IS NULL)";
            
            var resultSetCheckConvAuto = query.runSuiteQL({  //10 units
        ...
        if (scriptContext.type == scriptContext.UserEventType.VIEW || scriptContext.type == scriptContext.UserEventType.EDIT) {
            ...
            if (customerId) {
                var customerLookup = search.lookupFields({
                    ...
                    id: customerId,
                    columns: ['custentity_autodesk_cust_proc_readiness', 'companyname', 'custentity_autodesk_customer_type']
                });
                ...
                if (readinessStatus !== 'Completed' && readinessStatus !== 'No Setup Required' && autodeskCustomerType != 'Government' && queryResult && queryResult.length) {
                ...
                else if (autodeskCustomerType != 'Government' && queryResultCheckConvAuto && queryResultCheckConvAuto.length)
                {
        ...
        if (scriptContext.type == scriptContext.UserEventType.VIEW && rec.type == record.Type.ESTIMATE && (approvalStatus == APPROVED || subs == SUBS_CZ || subs == SUBS_SK)) {
            var isAutodesk = false;

            if (!listItemsArray.length) { return; }            
            ...
            if (isAutodesk /*&& oppStatus == 'Open'*/ && !autodeskValidationError) {
                ...
                var buttonName = "";
                if (autodeskQuoteStatus == PENDING && !autodeskQuoteNumber) { buttonName = "Create Quote" }
                else if (autodeskQuoteStatus == IN_PROGRESS) { buttonName = "Get Status" }
                else if (autodeskQuoteStatus == ACKNOWLEDGED) { buttonName = "Get Details" }
                else if (autodeskQuoteStatus == FINALIZED) { buttonName = "Finalize Quote" }

                suiteletUrl += '&quoteid=' + recId;

                //QUOTE SYNCHRONISATIOn BUTTON
                if (buttonName && ![QUOTE_DETAIL_STATUS["CANCELLED"], QUOTE_DETAIL_STATUS["FAILED"]].includes(parseInt(autodeskQuoteDetailStatus))) {
                ...
                //RESET QUOTE BUTTON
                if ((autodeskQuoteDetailStatus && RESETBUTTON_STATUS.includes(parseInt(autodeskQuoteDetailStatus)) || autodeskQuoteError) && autodeskQuoteStatus != COMPLETED) {
                ...
                if (autodeskQuoteStatus == PENDING && autodeskQuoteNumber && eligibleRolesSyncQuotePortal.includes(runtime.getCurrentUser().role)) 
                {
        ...
        if (scriptContext.type == scriptContext.UserEventType.VIEW && rec.type == record.Type.OPPORTUNITY /*&& approvalStatus == APPROVED*/ && ![STATUS.CLOSED_LOST, STATUS.QUOTECLOSED_WON, STATUS.CLOSED_WON, STATUS.CLOSED_LOST_STAGE8].includes(entityStatus)) {
