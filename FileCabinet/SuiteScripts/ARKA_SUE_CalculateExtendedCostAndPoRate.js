...
define([
  ...
  'N/error',
  'N/runtime',
], function (search, BBE_LIB_GenericFunctionsSS2, log, error, runtime) {
  // jshint ignore:line
    ...
    if (
      ...
      try {
        ...
        for (var i = 0; i < numItemLines; i++) {
          ...
          if (
            recordType != 'invoice' ||
            BBE_LIB_GenericFunctionsSS2.isNullOrEmpty(calculatedRate)
          ) {
            var itemType = record.getSublistValue({
              sublistId: 'item',
              fieldId: 'itemtype',
          ...
          for (i = 0; i < numItemLines; i++) {
            ...
            if (
              ...
              if (itemPricingType == 'Monthly' || itemPricingType == 'Annual') {
                ...
                if (
                  !BBE_LIB_GenericFunctionsSS2.isNullOrEmpty(poAmountMargin)
                ) {
                  log.debug({
                    title: 'porate + poAmountMargin + origPoRate',
                    details:
                      poRate + ' + ' + poAmountMargin + ' + ' + origPoRate,
                  });
                  ...
                  poRate =
                    (amount * (Number.parseFloat(poAmountMargin) / 100)) /
                    quantity;
                  estUnitCost = poRate;
                ...
                if (
                  !BBE_LIB_GenericFunctionsSS2.isNullOrEmpty(costEstimateType)
                ) {
                  log.debug('costEstimateType', costEstimateType);
                  if (costEstimateType != 'CUSTOM') {
                    if (
                      !BBE_LIB_GenericFunctionsSS2.isNullOrEmpty(
                        itemRates[i].extendedCost
                      )
                    ) {
                      record.setSublistValue({
                        sublistId: 'item',
        ...
        BBE_LIB_GenericFunctionsSS2.logError({
          err: e,
          title:
            'An Error has occured: Unable to calculate extended cost and po rate.',
  ...
  function setOrigEstimatedCostAndPoRate(
    ...
    if (
      ...
      !isNaN(poRate) &&
      isGov &&
      (recordType == 'estimate' || recordType === 'salesorder')
    ) {
    ...
    if (BBE_LIB_GenericFunctionsSS2.isNullOrEmpty(origEstUnitCost)) {
      ...
      if (itemPricingType == 'Annual') {
        extendedCost =
          origEstUnitCost * quantity * (calcTerm / NUMBER_OF_MONTHS);
      } else if (itemPricingType == 'Monthly') {
        ...
        record.setSublistValue({
          ...
          value:
            ((amount / appliedDiscount) *
              (Number.parseFloat(poAmountMargin) / 100)) /
            quantity,
          line: linenum,
        });
