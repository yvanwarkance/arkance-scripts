...
define([
  ...
  function fillEstiWith3Version(estimateThreeYr, estiLines) {
    for (var index = 0; index < estiLines.length; index++) {
      ...
      var itemEndDate = new Date(
        itemStrtDateFormatted.setMonth(itemStrtDateFormatted.getMonth() + 36)
      );
      ...
      log.debug({
        ...
        details:
          "Start: " +
          itemStrtDateFormatted.toDateString() +
          " , end: " +
          itemEndDate.toDateString(),
      });
  ...
  function beforeSubmit(scriptContext) {
    ...
    try {
      ...
      //CZ SK FIN
      if (subsidiary == 23 || subsidiary == 24 || subsidiary == 41 || true) {// for all subs
        ...
        if (!BBE_LIB_GenericFunctionsSS2.isNullOrEmpty(fromContractId)) {
          ...
          log.debug({title: 'Data check 1', details: 'missingContractItems: ' + missingContractItems.length});
      ...
      if (orderType == ORDER_TYPE_CONTRACT_RENEWAL && subsidiary != AGACAD ) {
        ...
        for (var index = 0; index < lines; index++) {
          ...
          if (!BBE_LIB_GenericFunctionsSS2.isNullOrEmpty(productLine)) {
            if (
              !BBE_LIB_GenericFunctionsSS2.isNullOrEmpty(
                productLine["custitem_product_line"][0]
              )
            ) {
              productLine = productLine["custitem_product_line"][0]["value"];
            } else {
      ...
      if (orderType == ORDER_TYPE_CONTRACT_RENEWAL && subsidiary == AGACAD) {
        ...
        for (var index = 0; index < lines; index++) {
          ...
          if (
            !BBE_LIB_GenericFunctionsSS2.isNullOrEmpty(
              item["custitem_item_categorization"]
            )
          ) {
            if (
              productLine == AGACAD_PRODUCTLINE &&
              item["custitem_item_categorization"][0]["value"] ==
              CATEGORIZATION_PERPETUAL
            ) {
              estimateRec.setSublistValue({
                sublistId: "item",
                fieldId: "price",
  ...
  function initiateScheduleScript() {
    try {
      ...
      scriptTask.deploymentId =
        "customdeploy_ssc_initializeestimate" +
        "_" +
        deploymentCount.toString();
      ...
      if (e.message.includes("INPROGRESS")) {
        ...
        NXLIB.trackScriptEvent(
          "ARKA_SUE_DuplicateEstimate",
          "Fail to update Estimate: " + e.message,
          658344
        );
      }
    }
  ...
  function afterSubmit(scriptContext) {
    ...
    try {
      ...
      if( type == 'opprtnty')//Generation for 3yr should not apply to opportunities
      {
          return;
      }
      ...
      if (SUBSIDIARY_ALLOWED.includes(subsidiary)) {
        ...
        if (contractTerm == 12 && estiOneYrLineCount == estiLines.length) {
          ...
          var estOneYr_endDateObj = new Date(
            estOneYr_strtDate.setMonth(estOneYr_strtDate.getMonth() + 36)
          );
          estOneYr_endDateObj = new Date(
            estOneYr_strtDate.setDate(estOneYr_strtDate.getDate() - 1)
          );
