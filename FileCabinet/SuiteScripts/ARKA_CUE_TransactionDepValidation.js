...
define(['N/query', 'N/ui/message', 'N/search', 'N/runtime'], function (query, msg, search, runtime) {
    ...
    function saveRecord(context) 
    {
      ...
      //log.error('Enter recTypeId', recTypeId);
        if (recTypeId)
        {
            ...
            if ( (!containSublistIds.length && recType != 'opportunity' ) || recType == 'inventorytransfer' || (shippingItemMethod && shippingItemCost && shippingItemCost > 0 && transToValidateShipItem.indexOf(recType) != -1))
            {
                ...
                {
                    validateDepartmentArea(departmentIdMain, departmentNameMain, areaIdMain, areaNameMain, errorMsg, true);
                }
            ...
            if (containSublistIds.length)
            {
                containSublistIds.forEach( sublistId => {
                    ...
                    for (let line = 0; line < lineCount && !(sublistId == lineSublistId.EXPENSE && transNotValidateExpense.indexOf(recType) != -1); line++)
                    {
                        let expenseAccId;
                        let lineKey = line + 1;
                        ...
                        if ((!deptId || !areaId) && ((accType && PNL_ACCOUNT_TYPE.indexOf(accType) != -1) || (expenseAccId && pnlAccounts.indexOf(parseInt(expenseAccId)) != -1) || sublistId == lineSublistId.ITEM) && emptyClassification.indexOf(lineKey) == -1)
                        {
        ...
        {
            if (departId && 
                (((!validDeptObj[departId] && Object.keys(validDeptObj).length) && !(isHeader && shippingItemMethod && shippingItemCost && shippingItemCost > 0 && transToValidateShipItem.indexOf(recType) != -1)) || 
            ...
            if (departId && validateDeptMessage.indexOf(lineRef) == -1 && Object.keys(areaDeptObj).length && areaId && areaDeptObj[departId] && areaDeptObj[departId].length && areaDeptObj[departId].indexOf(parseInt(areaId)) == -1)
            {
    ...
    function validateLine(context)
    {
        ...
        if (recType == 'journalentry' && sublistName == lineSublistId.LINE)
        {
            ...
            if (soLine && item && (!dept || !segmentLine))
            {
                ...
                segment = itemObjSubrec[CUSTOM_SEGMENT_FIELDID].length? itemObjSubrec[CUSTOM_SEGMENT_FIELDID][0].value: '';
    ...
    function validateEffectiveTranDate(currentRec)
    {
        ...
        if (postingPeriod || trandate)
        {
            ...
            toValid = (postingPeriod && postingPeriod >= EFFECTIVE_POSTING_PERIOD) || (!postingPeriod && formatedtranDate >= EFFECTIVE_TRANDATE);
        }
    ...
    function changeDateFormat(dateString)
    {
        ...
        return [date.getFullYear(), (date.getMonth() + 1).toString().padStart(2, '0'), (date.getDate()).toString().padStart(2, '0')].join('-');//YYYY-MM-DD
    }
    ...
    function fieldChanged(context){
      ...
      //log.error('context', context);
        ...
        if ((context.fieldId == 'category' || context.fieldId == 'account')
         && ( recType == 'expensereport' || context.line == 0) && context.sublistId == lineSublistId.EXPENSE) 
        {
