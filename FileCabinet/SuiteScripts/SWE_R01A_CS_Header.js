...
//Get Parameters
if(!SWE.R01A.CS.Header.Variables) SWE.R01A.CS.Header.Variables = 
{
...
{    
    ...
    if (SWE.R01A.CS.Header.Variables.isR01ECSEnabled) {
        ...
        if (stEventType == "edit" || stEventType == "copy") {
            ...
            if (!SWE.Library.misc.isUndefinedNullOrEmpty(stContractID)) {
                ...
                if (SWE.Renewals.Library.isRecordTypeRMA()) { //Issue 215816
    ...
    if(SWE.Renewals.Library.isRecordTypeRMA() && nlapiGetFieldValue('custbody_contract_name') !== null && nlapiGetFieldValue('custbody_contract_name') !== '' && (inpwhence || (inptransform && inpwhence))){       
...
SWE.R01A.CS.Header.validateField_Header = function(type, name) {
    ...
    if(name == 'custbody_tran_term_in_months' || name == startDateFldName || name == 'custbody_contract_name' || name == 'custbody_renewal_terms'){
        var logger = new SWE.Renewals.Library.Logger(true);
        ...
        if (SWE.Renewals.Library.isRecordTypeRMA() && name == 'custbody_contract_name') {
            if (SWE.Library.misc.isUndefinedNullOrEmpty(contractId)) {
                for (var i = 1; i <= nlapiGetLineItemCount('item'); i++) {
                    ...
                    if (SWE.Renewals.Library.itemCategoryRequiresContract(nlapiGetLineItemValue('item', 'custcol_item_category', i))) {
...
SWE.R01A.CS.Header.fieldChanged_Header = function(stType, stName, iLineNum) {
    ...
    if (SWE.R01A.CS.Header.Variables.isR01ACSEnabled) {
        ...
        if (stName == 'custbody_tran_term_in_months' &&
                (SWE.Renewals.Library.isRecordTypeQuote() || SWE.Renewals.Library.isRecordTypeSalesOrder() || SWE.Renewals.Library.isRecordTypeInvoice() || SWE.Renewals.Library.isRecordTypeRMA())) {
            var iTerm = nlapiGetFieldValue('custbody_tran_term_in_months');
            var stStartDateField = ( SWE.Renewals.Library.isRecordTypeRMA() === true ? 'custbody_swe_rma_header_start_date': 'startdate' );
            var stEndDateField = ( SWE.Renewals.Library.isRecordTypeRMA() === true ? 'custbody_swe_rma_header_end_date': 'enddate' );
            ...
            if (!SWE.Library.misc.isUndefinedNullOrEmpty(iTerm) && !SWE.Library.misc.isUndefinedNullOrEmpty(stStartDate)) {
                ...
                //Issue 355057
                if((stOrderType != constants.ORDER_TYPE_UPSELL && stOrderType != constants.ORDER_TYPE_DOWNSELL)
        ...
        if(stName == 'entity'){
            ...
            var stOrderType = nlapiGetFieldValue('custbody_order_type');
            if ((stOrderType == constants.ORDER_TYPE_RENEWAL || stOrderType == constants.ORDER_TYPE_RENEWAL_MANUAL)
    ...
    if (SWE.R01A.CS.Header.Variables.isR01ECSEnabled) {
        ...
        try {
            if (stName == 'custbody_contract_name') {
                ...
                SWE.R01A.CS.Header.setOrderType(SWE.Renewals.Library.isRecordTypeRMA() === false ? constants.ORDER_TYPE_UPSELL : constants.ORDER_TYPE_DOWNSELL);
                ...
                var fields = ['custrecord_contract_distributor',
                        ...
                        'custrecord_contract_status']; //Issue 204399
...
SWE.R01A.CS.Header.postSourcing_Header = function(type, name){
    ...
    if (SWE.R01A.CS.Header.Variables.AUTOSELECT_SINGLE_CONTRACT) { //Issue 199147
...
SWE.R01A.CS.Header.setOrderType = function(value) {
    var constants = SWE.Renewals.Library.Constants;
    if (SWE.Renewals.Library.isRecordTypeRefund()) {   //always set to downsell if RMA
...
SWE.R01A.CS.Header.salesOrderDateInit = function(eventType) {
    ...
    if (eventType == 'create' && !SWE.Library.misc.isUndefinedNullOrEmpty(contractId)) {
        ...
        nlapiSetFieldValue('custbody_order_type', (SWE.Renewals.Library.isRecordTypeRMA() === true ? constants.ORDER_TYPE_DOWNSELL : constants.ORDER_TYPE_NEW));
    }
