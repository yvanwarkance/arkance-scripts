...
<head>
	...
	<tr>
                        ...
                        <td line-height="8.5" colspan="12" style="padding-left: 20px;"><span class="footer-company-bold">
                                <!-- Additional restriction as per the request made by Olivier Cabeau -->
                              ${subsidiary.legalname?replace('Systems','')}
...
<body footer="nlfooter" footer-height="40pt" padding="0.5in 0.5in 0.5in 0.5in" size="Letter">
	...
	<table class="estimate-details">
		...
		<#if record.custbody_order_type == 'Contract - Renewal'>
        
          	<tr>
				<td class="object"><span class="bold-underline">OBJET :</span> Proposition Commerciale (Renouvellement de contrat)</td>
			</tr>
	...
	<p class="first-page-text">
		...
		Les logiciels proposés : Inventor, Fusion 360, AutoCAD, Revit, Vault, Civil 3D, 3ds Max, …
	</p>
	...
	<p class="first-page-text">
		Nous vous prions d'agréer l'expression de nos salutations distinguées. <!-- TODO: Add Primary Contact's Salutation -->
	</p>
	<br/>
  ...
  <!-- <body header="nlheader2" header-height="10%" footer="nlfooter" footer-height="40pt" padding="0.5in 0.5in 0.5in 0.5in" size="Letter"> -->
	<div class="estimate-title">
		...
		<#if record.custbody_order_type == 'Contract - Renewal'>
          	...
          	<p class="proposition-commerciale">Proposition Commerciale (Renouvellement de contrat)</p>
       ...
       <table style="width: 70%; margin-top: 10px;">  
        ...
        <#if record.custbody_end_user.custentity_autodesk_csn?has_content>
          	<tr>
				<td>Numéro CSN:</td>
				<td style="padding-top: 2px;">${record.custbody_end_user.custentity_autodesk_csn?string?replace(" ", "")?replace(",", "")}</td>
			</tr>
          </#if>
         ...
         <#if record.custbody_contract_manager_contact?has_content>
           ...
           <#if indexStart lt 0>
             ...
             <tr>
               <td>${record.custbody_contract_manager_contact@label}:</td>
               <td style="padding-top: 2px;">${ccm}</td>
             </tr>
             <tr>
               <td>Contract Manager adresse courriel:</td>
               <td style="padding-top: 2px;">${record.custbody_contract_manager_contact.email}</td>
             </tr>
          </#if>
          <#if record.custbody_order_type == 'Contract - Renewal'>
          	<tr>
				<td>${record.custbody_tran_printout_translation.custrecord_tran_period@label}:</td>
				<td style="padding-top: 2px;">${record.startdate} - ${record.enddate}</td>
			</tr>
          </#if>
		</table>
   ...
   <!-- CR GOV --> 
      <#assign isGov = false />
      <#if record.custbody_end_user.custentity1?has_content>
        <#assign industryList = record.custbody_end_user.custentity1?split(',') />
		...
		<table class="itemtable" style="width: 100%; margin-top: 10px;">
			<!-- start items -->
				...
				<tr> 
					...
					<#assign inlineDiscountPercentage = item.custcol_inline_discount />
                  ...
                  <#if item.custcol_inline_discount?has_content>
					...
					<td class="item-table-item" align="left" width="35%">
                      ...
                      <br />${item.description} <#if isGov == true && subsidiary.id == '3' && item.itemtype == 'NonInvtPart'> (Conditions Gouvernement) </#if>  <!-- CR GOV --> 
                      <#if item.custcol_autodesk_contract_number?has_content>
                        <p>
                          (Numéro Contrat: ${item.custcol_autodesk_contract_number})
                        </p>
                      </#if>
                      <#if hasInlineDiscount == true>
                        <p>
                          (Prix standard: ${item.custcol_rate_before_discount})
                        </p>
                      </#if>
                      <#if item.custcol_swe_contract_start_date?has_content && item.custcol_swe_contract_end_date?has_content>
                        <#if record.startdate?trim == ''|| item.custcol_swe_contract_start_date != record.startdate || item.custcol_swe_contract_end_date != record.enddate>
                        	<p>(Période: ${item.custcol_swe_contract_start_date} - ${item.custcol_swe_contract_end_date} )</p>
                      	</#if>
                      </#if>
                    </td>
			...
			<tr>
				...
				<td class="item-table-total-value" align="right" width="20%">${record.taxtotal}</td>
			</tr>
			<tr>
				<td class="item-table-empty-total" colspan="2" align="left" width="60%">&nbsp;</td>
				<td class="item-table-total-label" colspan="2" align="right" width="20%">Montant total (TTC)</td>
				<td class="item-table-total-value" align="right" width="20%">${record.total}</td>
			</tr>  -->
		</table>
	</#if>
	...
	<p class="payment-condition-value" style = "page-break-after: always;">${record.terms}</p>
  ...
  <tr>
    <th class = "signature" style="text-align:center">Fonction..................................................</th>
         ...
         <tr>
    <th class = "signature" style="text-align:center">Adresse courriel de facturation:</th>
  </tr>
               <tr>
    <th class = "signature" style="text-align:center">.................................................................</th>
  </tr>
  ...
  <tr>
	...
	<table class="forfait-hotline-table">
      ...
      <!-- Installation (Line commented according to the request made by Olivier Cabeau-->
		<!-- <tr>
			<td class="forfait-hotline-table-data-multirow" rowspan="3" align="left" width="25%">Installation</td>
			<td class="forfait-hotline-table-data" align="left" width="35%">Aide à la résolution des problèmes d’installation du logiciel souscrit </td>
			
		</tr>
		<tr>
			<td class="forfait-hotline-table-data" align="left" width="35%">Installation de correctif (Hotfix et Service Pack)</td>
			
		</tr>
		<tr>
			<td class="forfait-hotline-table-data" align="left" width="35%">Installation des plug-in Arkance</td>
      ...
      <!-- Activation licences Gestion de Compte Autodesk -->
		<tr>
			<td class="forfait-hotline-table-data-multirow" rowspan="4" align="left" width="25%">Activation des licences</td>
			<td class="forfait-hotline-table-data" align="left" width="35%">Assistance à l’activation des produits</td>
			
		</tr>
		<tr>
          <!-- Line commented according to the request made by olivier cabeau -->
			<!-- <td class="forfait-hotline-table-data" align="left" width="35%">Gestion (Répartition et attribution) des utilisateurs et des licences (1)</td> -->
		...
		<tr>
			<!-- <td class="forfait-hotline-table-data" align="left" width="35%">Résolution des problèmes de Proxy/ Firewall pour la connexion de la licence</td> -->
		...
		<tr>
			<td class="forfait-hotline-table-data-multirow" rowspan="3" align="left" width="35%">Utilisation du logiciel</td>
		...
		<tr>
            <!-- Line commented according to the request made by olivier cabeau -->
			<td class="forfait-hotline-table-data" align="left" width="35%">Assistance à la conception</td>
		...
		<!-- Accès à la Newsletter et aux évènements Arkance Systems -->
        <!-- Line commented according to the request made by olivier cabeau -->
		<!-- <tr>
			<td class="forfait-hotline-table-data-multirow" rowspan="3" align="left" width="25%">Accès à la Newsletter et <br/> aux évènements <br/> Arkance Systems</td>
			<td class="forfait-hotline-table-data" align="left" width="35%">Accès à la Newsletter Tips &amp; Tricks</td>
			
		</tr>
		<tr>
			<td class="forfait-hotline-table-data" align="left" width="35%">Accès à l’ensemble des webinaires Arkance (3)</td>
			
		</tr>
      	<tr>
			<td class="forfait-hotline-table-data" align="left" width="35%">Partage sur les nouveautés Autodesk</td>
	...
	<p class="forfait-hotline-details-bold">
		(1) : En fonction de l'intensité des demandes, une formation avancée ou spéciﬁque pourra être proposée.<br />
	</p>

	<p class="forfait-hotline-details">
      <!-- Modifications made as per requested by Olivier cabeau -->
		Le service d’assistance téléphonique est <span class="forfait-hotline-details-emphasis">accessible</span> du lundi au vendredi de 9h00 à 12h00 et 13h00 - 17h00 sauf les jours fériés. 
      <br/>
      <br/>Dans le cadre de notre support technique, toutes les interventions et prestations fournies sont limitées à une durée maximale de 45 minutes par ticket. Au-delà de cette période, nous vous orienterons vers votre interlocuteur commercial afin d'évaluer la possibilité d'un besoin éventuel en formation ou en consulting.
	</p>

	<p class="forfait-hotline-details" style="page-break-after: always;">
		Faites votre demande sur :
		<ul>
			<li>Notre site Internet : www.arkance.world – rubrique « Service – Assistance Technique »</li>
			<li>Par Téléphone au 01.39.44.18.18 choix N°3</li>
			<li>Par courriel : support.technique@arkance.world</li>
		</ul>
	</p>
