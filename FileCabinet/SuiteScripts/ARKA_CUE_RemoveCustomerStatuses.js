...
define([], function () {
    ...
    function pageInit(context) {
        ...
        let fieldToHide = {
            estimate: ['08 - Quote Closed Won', 'Stage 1 - Prospecting', 'Stage 2 - Qualification', 'Stage 3 - Solution Design'],
            opprtnty: ['Stage 4 - Decision Pending/Negotion', 'Stage 5 - Verbal Agreement', 'Stage 6 - Order Validation','Stage 7 - Final/Closed/Won','10 - Dormant','11 - Lost Customer']
        };
      //let fieldToHide2 = {estimate: [],opprtnty: []}; 
            ...
            for (m of mutationList) {
                if (m.attributeName == 'class' && m.type == 'attributes') {
                    if (m.target.classList.contains('uir-field-active')) {
                        ...
                        {
                            if (fieldToHide[recType].includes(opt.innerHTML) && !opt.classList.contains('dropdownSelected')) {
        ...
        if (recType == 'estimate') {
            if (context.mode == 'copy') {
                ...
                var fieldToEmpty = ['custbody_autodesk_transaction_id', 'custbody_autodesk_quote_detail_status',
                    'custbody_autodesk_quote_number', 'custbody_autodesk_error', 'custbody_autodesk_quote_id']
                var variableEmpty = [{ id: 'text', value: null, data: fieldToEmpty },
